...
1.  Install a `nix` distribution in your system and enable flakes. The snippet below will install [Determinate Nix](https://docs.determinate.systems/) which have flakes pre-enabled.
    ...
    ```
2. Determine which target you want to use, and run below (the example below will use the `cachyos` target). This may take a while. 
    ```bash
    nix run github:nix-community/nh -- home switch github:selubi/machine -c cachyos
    ```
